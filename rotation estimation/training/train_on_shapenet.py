...
dataset = shapenet.ShapeNetV2(data_dir=ShapeNet_PATH,
                              config=cfg,
                              x_bound=(-0.02, 0.02),
                              y_bound=(-0.02, 0.02),
                              scale_jitter=(0.05, 0.25),
                              dist_jitter=(0.5, 1.5),
                              aug_rescale_jitter=(0.2, 0.8),
                              aug_patch_area_ratio=0.2,
                              aug_patch_max_num=1,
                              aug_guassian_std=0.02,
                             )
...
triple_margin = cfg.RANKING_MARGIN

max_angle_margin = math.pi / 2


variant_name = "net_{}x{}_{}epochs".format(img_hei, img_wid, max_epochs)
...
SmoothL1_sum = torch.nn.SmoothL1Loss(reduction='sum', beta=0.001)
SmoothL1_mean = torch.nn.SmoothL1Loss(reduction='mean', beta=0.001)

Triple_CosineDist = torch.nn.TripletMarginWithDistanceLoss(
    distance_function=lambda x, y: 1.0 - F.cosine_similarity(x, y),
    margin=triple_margin,
    swap=True,
    reduction='mean')
...
for epoch in range(max_epochs):
    for train_data in data_loader:
        ...
        valid_img_idx = (valid_anc_idx * valid_inp_idx * valid_out_idx).bool()
        
        anc_maps_GT = batchview.bv2b(train_data['anchor']['map'])
        inp_maps_GT = batchview.bv2b(train_data['inplane']['map'])
        out_maps_GT = batchview.bv2b(train_data['outplane']['map'])
        
        inp_imgs_aug = batchview.bv2b(train_data['inplane']['aug_img'])
        out_imgs_aug = batchview.bv2b(train_data['outplane']['aug_img'])
        inp_maps_aug = torch.zeros_like(inp_imgs_aug)
        inp_maps_aug[inp_imgs_aug>0] = 1
        out_maps_aug = torch.zeros_like(out_imgs_aug)
        out_maps_aug[out_imgs_aug>0] = 1
        
        anc_map_pixels = anc_maps_GT.view(anc_maps_GT.shape[0], -1).sum(1)
        inp_gt_map_pixels = inp_maps_GT.view(inp_maps_GT.shape[0], -1).sum(1)
        inp_aug_map_pixels = inp_maps_aug.view(inp_maps_aug.shape[0], -1).sum(1)
        inp_aug_visib_frac = inp_aug_map_pixels / inp_gt_map_pixels

        out_gt_map_pixels = out_maps_GT.view(out_maps_GT.shape[0], -1).sum(1)
        out_aug_map_pixels = out_maps_aug.view(out_maps_aug.shape[0], -1).sum(1)
        oup_aug_visib_frac = out_aug_map_pixels / out_gt_map_pixels

                
        valid_map_idx = ((anc_map_pixels > cfg.MIN_img_PIXELS).bool() & 
                          (inp_aug_map_pixels > cfg.MIN_img_PIXELS).bool() &
                          (out_aug_map_pixels > cfg.MIN_img_PIXELS).bool() & 
                          (inp_aug_visib_frac > cfg.VISIB_FRAC).bool() & 
                          (oup_aug_visib_frac > cfg.VISIB_FRAC).bool())


        valid_idx = valid_map_idx & valid_img_idx
        num_samples = valid_idx.sum()
        if num_samples < 4:
            continue

        anc_imgs_GT = batchview.bv2b(train_data['anchor']['img'])[valid_idx]
        inp_imgs_GT = batchview.bv2b(train_data['inplane']['img'])[valid_idx]
        out_imgs_GT = batchview.bv2b(train_data['outplane']['img'])[valid_idx]
        
        inp_imgs_aug = batchview.bv2b(train_data['inplane']['aug_img'])[valid_idx]
        out_imgs_aug = batchview.bv2b(train_data['outplane']['aug_img'])[valid_idx]
        ...
        try:
            ...
            inp_gt_transform = preprocess.residual_inplane_transform3(gt_t=zoom_inplane_T,
                                                                    ...
                                                                    target_dist=target_zoom_dists,
                                                                    device=DEVICE)
            del (anc_imgs_GT, inp_imgs_GT, out_imgs_GT, inp_imgs_aug, out_imgs_aug)
        except:
            logger.warning('skip zooming preprocess ...')
            continue
        
        gt_full_T = zoom_inplane_T.to(DEVICE)
        gt_theta = inp_gt_transform[:, :2, :3].to(DEVICE)
        gt_dz_offset = inp_gt_transform[:, 2:3, 2].to(DEVICE)
        ...
        pd_stn_imgs = preprocess.spatial_transform_2D(x=zoom_anchor_GT.to(DEVICE),
                                                        theta=pd_theta.to(DEVICE), 
                                                        mode=cfg.ZOOM_MODE, 
                                                        padding_mode="zeros", 
                                                        align_corners=False)
        gt_stn_imgs = preprocess.spatial_transform_2D(x=zoom_anchor_GT.to(DEVICE), 
                                                        theta=gt_theta.to(DEVICE), 
                                                        mode=cfg.ZOOM_MODE, 
                                                        padding_mode="zeros", 
                                                        align_corners=False)

        gt_dxdy_offset = gt_theta[:, :2, 2]
        pd_dxdy_offset = pd_theta[:, :2, 2]

        tsl_loss = SmoothL1_mean(pd_dxdy_offset, gt_dxdy_offset)

        pd_T_offset = torch.cat([pd_dxdy_offset, gt_dz_offset.to(pd_dxdy_offset.device)], dim=1)
        pd_init_T = pd_init_T.type(pd_T_offset.dtype)
        ...
        img_L1_loss = SmoothL1_sum(pd_stn_imgs, gt_stn_imgs) / num_samples

        view_cosim = Cosine_Sim(pd_stn_imgs.view(num_samples, -1), gt_stn_imgs.view(num_samples, -1))
        view_cosim = torch.clamp((view_cosim + 1.0) / 2.0, min=0.0+eps, max=1.0-eps)
        view_dissim = -torch.log(view_cosim)
        img_sim_loss = view_dissim.mean()

        gt_Rz_rot = gt_theta[:, :2, :2]
        pd_Rz_rot = pd_theta[:, :2, :2]
        ...
        inp_vp = anc_extrinsic_GT[:, 2, :3].to(DEVICE)
        out_vp = out_extrinsic_GT[:, 2, :3].to(DEVICE)
        ...
        if iter_steps <= 10 or iter_steps % 100 == 0:
            ...
            eval_pd_grid = torchvision.utils.make_grid(viz.colorize_tanh_img(
                                                        preprocess.spatial_transform_2D(x=zoom_anchor_GT[:4], 
                                                        theta=eval_theta, 
                                                        mode='nearest')), nrow=2)

            anc_gt_grid = torchvision.utils.make_grid(viz.colorize_tanh_img(zoom_anchor_GT[:4]), nrow=2)
            oup_gt_grid = torchvision.utils.make_grid(viz.colorize_tanh_img(zoom_outplane_GT[:4]), nrow=2)

            inp_aug_grid = torchvision.utils.make_grid(viz.colorize_tanh_img(zoom_inplane_aug[:4]), nrow=2)
            oup_aug_grid = torchvision.utils.make_grid(viz.colorize_tanh_img(zoom_outplane_aug[:4]), nrow=2)

            stn_pd_grid = torchvision.utils.make_grid(viz.colorize_tanh_img(pd_stn_imgs[:4]), nrow=2)
            stn_gt_grid = torchvision.utils.make_grid(viz.colorize_tanh_img(gt_stn_imgs[:4]), nrow=2)
